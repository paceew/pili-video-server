$(document).ready(function() {
    ...
    initPage(function() {
        if (listedVideos !== null) {
            ...
            selectVideo(listedVideos[0]['video_id']);
        }
        ...
        $("#submit-comment").on('click', function() {
            ...
            postComment(currentVideo['video_id'], content, function(res, err) {
                if (err !== null) {
                ...
                if (res === "ok") {
                    ...
                    refreshComments(currentVideo['video_id']);
                }
            });
        });
    });
    ...
    $("#uploadform").on('submit', function(e) {
        ...
        createVideo(vname, function(res, err) {
            ...
            $.ajax({
                url: 'http://' + window.location.hostname + ':8080/video/' + obj['video_id'],
    ...
    $(".video-item").click(function() {
        var url = 'http://' + window.location.hostname + ':9000/video/' + this.id
...
function initPage(callback) {
    getUserId(function(res, err) {
        ...
        listAllVideos(function(res, err) {
            ...
            var obj = JSON.parse(res);
            console.log(obj)
            listedVideos = obj['videos_list'];
            obj['videos_list'].forEach(function(item, index) {
                var ele = htmlVideoListElement(item['video_id'], item['video_name'], item['video_ct']);
                $("#items").append(ele);
            });
            callback();
        });
...
function selectVideo(vid) {
    var url = 'http://' + window.location.hostname + ':8080/video/' + vid
    var video = $("#curr-video");
    console.log(video)
    console.log(url)
    $("#curr-video:first-child").attr('src', url);
    $("#curr-video-name").text(currentVideo['video_name']);
    $("#curr-video-ctime").text('Uploaded at: ' + currentVideo['video_ct']);
    //currentVideoId = vid;
    refreshComments(vid);
}
...
function refreshComments(vid) {
    listAllComments(vid, function(res, err) {
        ...
        if (obj['comments_list'] === null) {
            $("#comments-total").text('0 Comments');
        } else {
            $("#comments-total").text(obj['comments_list'].length + ' Comments');
        }
        obj['comments_list'].forEach(function(item, index) {
            console.log(item)
            var ele = htmlCommentListElement(item['comment_id'], item['AuthorName'], item['content']);
